...
def partition(seq, p, r):
    ...
    seq[i+1], seq[r-1] = seq[r-1], seq[i+1]
    return i + 1
...
def randomized_partition(seq, p, r):
    ...
    i = random.randint(p, r-2)
    seq[i], seq[r-2] = seq[r-2], seq[i]
...
def double_partition(seq, p, r, q):
    """
...
def double_quick_sort(seq_a, seq_b, p, r):
    ...
    """
    if p < (r-1):
        q = double_partition(seq_a, p, r, seq_b[r-1])
        double_partition(seq_b, p, r, seq_a[q])
        double_quick_sort(seq_a, seq_b, p, q-1)
